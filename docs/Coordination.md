...
`endpoint` + `username` / `password` + `db` form works in both
places. Only `endpoint` is required; the other fields default to
the values shown. The lease store is small and ephemeral, so it
can share the Redis instance (and logical database) used by the
Redis cache backend.
